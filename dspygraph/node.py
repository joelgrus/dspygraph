...
class Node(ABC):
    ...
    def __call__(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        print(
            f"[{self.name}] Starting execution {self._execution_count} (id: {execution_id[:8]})"
        )
    ...
    def ensure_compiled(self, compile_path: str | None = None) -> None:
        ...
        raise RuntimeError(
            f"Node '{self.name}' requires compilation but no compiled module found"
        )
