...
class DetailedCallback(BaseCallback):
    ...
    def on_module_start(self, call_id, instance, inputs):
        print(
            f"📝 Module Start - Call ID: {call_id[:8]}, Module: {type(instance).__name__}"
        )
        print(
            f"   Inputs: {list(inputs.keys()) if isinstance(inputs, dict) else 'N/A'}"
        )
    ...
    def on_lm_start(self, call_id, instance, inputs):
        print(
            f"🤖 LM Call Start - Call ID: {call_id[:8]}, Model: {getattr(instance, 'model', 'Unknown')}"
        )
