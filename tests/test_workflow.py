...
from dspygraph import END, START, Graph, Node
...
class SimpleTestNode(Node):
    ...
    def __init__(
        self, name: str, output_key: str = "output", output_value: str = "result"
    ):
...
class TestGraph:
    ...
    def test_add_edge_with_condition(self):
        ...
        assert condition({"test": True})
        assert not condition({"test": False})
    ...
    def test_add_conditional_edges(self):
        ...
        with patch("builtins.print"):
            ...
            graph.add_conditional_edges(
                "node1", {"path1": "node2", "path2": "node3", "end": END}, router
            )
        ...
        edge_conditions = [
            ...
            for from_node, to_node, condition in graph.edges
        ]

        # All should be from node1
        for from_node, _, condition in edge_conditions:
            assert from_node == "node1"
            assert condition is not None
    ...
    def test_conditional_workflow_execution(self, mock_track_usage):
        ...
        with patch("builtins.print"):
            ...
            graph.add_conditional_edges(
                "classifier", {"path1": "path1", "path2": "path2", "end": END}, router
            )
